...
# from apps.commerce.models import Coupon, OrderItem
...
#         seed_products = Product.objects.filter(sku__startswith="ANNA-")
#         blocked_seed_product_ids = OrderItem.objects.filter(
#             product_id__in=seed_products.values_list("id", flat=True)
#         ).values_list("product_id", flat=True).distinct()

#         # Remove only seed products that are not referenced by order history.
#         seed_products.exclude(id__in=blocked_seed_product_ids).delete()
#         seed_products.filter(id__in=blocked_seed_product_ids).update(is_active=False)
